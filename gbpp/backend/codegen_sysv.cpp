...
namespace gbpp {
    ...
    namespace {
        ...
        const int REG_RSI = 4; const int REG_RDI = 5; const int REG_R8 = 6; const int REG_R9 = 7;
        const int REG_R10 = 8; const int REG_R11 = 9; const int REG_R12 = 10; const int REG_R13 = 11;
        const int REG_R14 = 12; const int REG_R15 = 13;
        const int REG_RBP = 14; const int REG_RSP = 15;
    }

    class CodeGenSystemV : public CodeGen {
    ...
    private:
        const std::vector<int> argRegs = { REG_RDI, REG_RSI, REG_RDX, REG_RCX, REG_R8, REG_R9 };
        ...
        std::string getRegName(int id, int bytes) {
            ...
            if (id == REG_RBP) return (bytes == 8) ? "rbp" : "ebp";
            if (id == REG_RSP) return (bytes == 8) ? "rsp" : "esp";
        ...
        void genFunction(IRFunction& fn, Emitter& emitter) {
            ...
            tri.callerSaved = { REG_RAX, REG_RCX, REG_RDX, REG_RSI, REG_RDI, REG_R8, REG_R9, REG_R10, REG_R11 };
            tri.calleeSaved = { REG_RBX, REG_R12, REG_R13, REG_R14, REG_R15, REG_RBP };
            ...
            for (const auto& block : fn.blocks) {
                for (const auto& inst : block->instructions) {
                    ...
                    if (inst.op == OpCode::CALL) {
                        ...
                        if (inst.imm >= 6) hasStackArgs = true;
                    }
                    else {
                        if (inst.src1 != -1) usedAsReg.insert(inst.src1);
                        if (inst.src2 != -1) usedAsReg.insert(inst.src2);
            ...
            int shadowSpace = 0;
            if (hasCall) {
                if (maxCallArgs > 6) shadowSpace += (maxCallArgs - 6) * 8;
            }
                ...
                for (const auto& inst : irBlock->instructions) {
                    switch (inst.op) {
                        ...
                        case OpCode::CALL: {
                            ...
                            for (size_t i = 6; i < inst.args.size(); ++i) {
                                MachineOperand src = argSrcs[i];
                                int stackOffset = ((i - 6) * 8);
                            ...
                            for (size_t i = 0; i < std::min((size_t)6, inst.args.size()); ++i) {
                                int targetReg = argRegs[i];
                                bool conflict = false;

                                for (size_t j = i + 1; j < std::min((size_t)6, inst.args.size()); ++j) {
            ...
            for (size_t b = 0; b < mirFn.blocks.size(); ++b) {
                ...
                for (const auto& inst : mb.insts) {
                    if (inst.opcode == MInstOpcode::X86_INLINE_ASM) continue;
    ...
    std::unique_ptr<CodeGen> createSystemV() { return std::make_unique<CodeGenSystemV>(); }
}